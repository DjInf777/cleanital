...
export function sanitize(value, limit = 500) {
  return String(value ?? '')
    .replace(/[\u0000-\u001f\u007f]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
...
export function validateSubmission(kind, input) {
  ...
  if (
    ...
    started <= 0 ||
    age < minimumCompletionMs ||
    age > maximumFormAgeMs
  ) {
    return { ok: false, message: 'Please reload the form, review it, and try again.' };
  }

  const clean = {};
  for (const [key, limit] of Object.entries(max)) clean[key] = sanitize(input[key], limit);
  clean.consent = sanitize(input.consent, 10);
  for (const field of requiredByKind[kind]) {
    if (!clean[field]) return { ok: false, message: 'Complete all required fields.' };
  }
  ...
  if (clean.consent !== 'yes') {
    return { ok: false, message: 'Consent is required to respond to this request.' };
  }
...
function ref() {
  ...
  return `CLN-${day}-${randomBytes(5).toString('hex').toUpperCase()}`;
}

function hashIp(ip, key) {
  return createHmac('sha256', key).update(ip || 'unknown').digest('hex').slice(0, 24);
}

export async function processSubmission({ kind, input, ip, userAgent, storageDir, ipHashKey, notify }) {
  const validated = validateSubmission(kind, input);
  if (!validated.ok) return validated;
  if (!storageDir || !ipHashKey || ipHashKey === 'replace-with-a-secret') {
    return { ok: false, message: 'The request service is not configured. No success was recorded.' };
  }
  ...
  const record = {
    ...
    status: 'received',
    data: validated.data,
    abuse: { ipHash: hashIp(ip, ipHashKey), userAgent: sanitize(userAgent, 300) }
  };
